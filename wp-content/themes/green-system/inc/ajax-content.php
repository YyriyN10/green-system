<?php
  ...
  <defs>
	...
	add_action('wp_ajax_more_ess_project', 'more_ess_project_callback');
	add_action('wp_ajax_nopriv_more_ess_project', 'more_ess_project_callback');

	function more_ess_project_callback(){
		...
		$projectsList = new WP_Query( $projectsMore );
 ...
 */
  ...
  <defs>
	...
	add_action('wp_ajax_more_cases_in_case', 'more_cases_in_case_callback');
	add_action('wp_ajax_nopriv_more_cases_in_case', 'more_cases_in_case_callback');

	function more_cases_in_case_callback(){

	  $allCases = $_POST['allCases'];
	  $iteration = $_POST['iteration'];

	  if ( $iteration === 0 ){
	    $offset = 4;
    }else{
	    $offset = ( $iteration * 8 ) + 4;
    }
      ...
      'offset' => $offset
		...
		$projectsList = new WP_Query( $catArgs  );
